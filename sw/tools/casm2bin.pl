...
die "Usage: perl do_it.pl <asm_file> <c_file> <linker_script> <target_bin>" if (@ARGV != 4);
...
# Remove files.
system("rm -fv /tmp/asm_file.o /tmp/c_file.asm /tmp/c_file.o /tmp/elf_file.elf $target $bin_file");
